...
- **YouTube経由(メイン導線)**: YouTubeアプリで「共有」→「現在の再生位置を共有」にチェック → 言の葉帖を選択。
  ...
  入力するのは気づいたフレーズ本文だけ。
- **URL貼り付け(PC向け)**: キャプチャ画面のURL欄にYouTube URLを貼ると同じ自動取得が走る
  (「共有 → URLをコピー」したものをそのまま貼ればOK。`?t=` も引き継がれる)。
- **手動入力(サブ導線)**: 「＋」から。フレーズ本文のみ必須、出典メモ(「会議」等)は任意。
- **キーボード**: キャプチャは Ctrl+Enter で保存、復習は ←/→ で「まだ未使用/使ってみた」、
  流し見ビューは Esc で閉じる。

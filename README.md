...
### ランディングページ(シリンダーギャラリー)

URLをそのまま開くと(ハッシュ無し)、[meech213.com](https://www.meech213.com/) を参考にした
**円筒状に回転するサムネイルの玄関ページ**が表示される。

- 自動でゆっくり回転し、ドラッグ/ホイールで回せる(慣性付き)
- 下部中央に散らし配置のナビ(はじめる / 一覧 / 復習 / 仕分け / 拾う)
- カードをタップするとギャラリーへ

**サムネイルの差し替え方**: 現在はストック画像(生成した和紙調プレースホルダ)。
`assets/landing/` に好きな画像を置き、`js/landing-images.js` の配列を編集するだけ
(同名ファイルで上書きするならコード編集も不要)。枚数は自由。
